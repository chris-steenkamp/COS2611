...
template <class Type>
int queueType<Type>::_queueFront() const
{
    return queueFront;
}

template <class Type>
int queueType<Type>::queueBack() const
{
    return queueRear;
}
...
void queueType<Type>::moveNthFront(int n)
{
    ...
    if (n == 1)
        return; //item is already at front
    ...
    {
        //find item at position n
        if (i == (n - 1))
            nth = front();
        else //other items get transferred to
            //temp queue in original order
            temp.addQueue(front());

        deleteQueue();
    }

    //add nth item to front of queue
    addQueue(nth);

    items = temp.size();

    //add remaining items back to queue in order
    for (int i = 0; i < items; ++i)
    {
        addQueue(temp.front());
        temp.deleteQueue();
